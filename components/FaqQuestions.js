  ...
  return (
    <>
      <Wrapper className="grid grid-cols-1 md:grid-cols-2 gap-y-6 justify-between gap-x-4 relative pt-0 pb-6 pl-0 pr-0">
        {!showMore && (
          <div className="bg-gradient-to-t from-white/50 from-20%  to-transparent  absolute w-full h-[40%] bottom-0"></div>
        )}
...
const FaqCard = ({ name, answer, icon, index }) => {
  return (
    <div className="flex items-start text-black text-justify">
      <div className="mr-4 border-gray-400 p-2 px-2 border rounded-xl ">
        {index + 1}
      </div>
